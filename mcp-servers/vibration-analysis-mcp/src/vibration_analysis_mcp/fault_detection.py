...
Uses vibration spectral features to identify common mechanical faults:
    - Unbalance (1× shaft speed dominant)
    - Misalignment (2× shaft speed, axial component)
    - Mechanical looseness (many harmonics of shaft speed)
    - Bearing faults (BPFO / BPFI / BSF / FTF in envelope spectrum)
    - Gear mesh faults (gear mesh frequency and sidebands)
...
class ShaftFeatures:
    ...
    amp_1x: float       # 1× amplitude
    amp_2x: float       # 2× amplitude
    amp_3x: float       # 3× amplitude
...
def classify_faults(
    ...
    # --- Unbalance: dominant 1× with low 2× ---
    ...
    if ratio_1x > 3.0 and features.amp_1x > 2.0 * features.amp_2x:
        ...
        diagnoses.append(FaultDiagnosis(
            ...
            confidence=conf,
            description="Mass unbalance detected – dominant 1× shaft speed component.",
            evidence=[
                f"1× amplitude = {features.amp_1x:.4f} ({ratio_1x:.1f}× RMS)",
                f"2× amplitude = {features.amp_2x:.4f} (ratio 1×/2× = {features.amp_1x / max(features.amp_2x, 1e-12):.1f})",
            ],
            recommendations=[
    ...
    # --- Misalignment: significant 2× (and sometimes 3×) ---
    if ratio_2x > 2.5 and features.amp_2x > 0.5 * features.amp_1x:
        ...
        diagnoses.append(FaultDiagnosis(
            ...
            confidence=conf,
            description="Shaft misalignment suspected – elevated 2× component.",
            evidence=[
                f"2× amplitude = {features.amp_2x:.4f} ({ratio_2x:.1f}× RMS)",
                f"2×/1× ratio = {features.amp_2x / max(features.amp_1x, 1e-12):.2f}",
                f"3× amplitude = {features.amp_3x:.4f}",
            ],
            recommendations=[
    ...
    n_significant = sum(
        1 for a in [features.amp_1x, features.amp_2x, features.amp_3x]
        if a / rms > 1.5
    )
    if n_significant >= 3 or (features.amp_half_x / rms > 1.5):
        ...
        if features.amp_half_x / rms > 1.5:
            evidence.append(f"Sub-harmonic 0.5× = {features.amp_half_x:.4f}")
        diagnoses.append(FaultDiagnosis(
        ...
        for fault_key, label in [
            ...
            if result and result.get("confidence", "none") != "none":
                ...
                diagnoses.append(FaultDiagnosis(
                    ...
                    evidence=[
                        f"Harmonics detected: {result.get('harmonics_detected', 0)}/{result.get('harmonics_checked', 3)}",
...
def generate_diagnosis_summary(
    ...
    if iso_assessment:
        ...
        lines.append(f"**Overall Severity (ISO 10816):** Zone {zone} — {vel:.2f} mm/s RMS — {desc}\n")
    
    lines.append("### Detected Conditions\n")
    for i, d in enumerate(diagnoses, 1):
        icon = {"high": "🔴", "medium": "🟡", "low": "🟢", "none": "⚪"}.get(d.confidence, "⚪")
        lines.append(f"#### {i}. {d.fault_type.replace('_', ' ').title()} [{d.confidence.upper()}] {icon}\n")
