...
def compute_psd(
    ...
    Returns:
        Dictionary with 'frequencies' (Hz) and 'psd' (g²/Hz or V²/Hz)
    """
    if noverlap is None:
        noverlap = nperseg // 2

    freqs, psd = sig.welch(data, fs=fs, nperseg=nperseg, noverlap=noverlap, window=window)
...
def find_peaks_in_spectrum(
    ...
    # Find peaks
    peak_indices, properties = sig.find_peaks(
        magnitude_db,
